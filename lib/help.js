export function renderHelp(style) {
  return [
    'Usage: ' +
      ...
      ' <install [plugin-path]|cache check|cache sync|status|doctor> ' +
      style.dim('[options]'),
    '',
    ...
    '  --marketplace-path <file> install through a local .agents/plugins/marketplace.json',
    ...
    '  --dry-run                plan install/sync without writing',
    ...
    'install uses Codex 0.153.x; native child failures retain their exit code. Authentication/activation may remain unknown.',

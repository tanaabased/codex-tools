export function renderHelp(style) {
  return [
    'Usage: ' +
      ...
      ' <cache check|cache sync|status|doctor|validate> ' +
      style.dim('[options]'),
    '',
    style.heading('Options') + ':',
    '  --repo-root <path>        local source ' + style.dim('[default: current directory]'),
    '  --python <executable>    validator Python >=3.10 with PyYAML >=6,<7',
    '  --repository-checks <file> explicitly execute trusted check descriptors (JSON)',
    ...
    '  CODEX_TOOLS_PYTHON          same as --python (default: python3)',
    ...
    'validate checks a standalone plugin; no cache or package.json is required.',
    'Validation exit: 0 valid; 1 invalid/unsupported/check failure; 2 dependency or invocation failure.',
    'Repository checks run only with --repository-checks, never through discovery or environment.',
  ].join('\n');
}

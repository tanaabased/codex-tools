...
import { runNative } from './install.js';
...
export async function refreshPlugin(
  options = {},
  { env = process.env, native = runNative, now = new Date() } = {},
) {
  ...
  try {
    for (const operation of result.plan) {
      ...
      switch (operation.operation) {
        case 'preflight':
          result.codexVersion = await call(operation.argv, false);
          if (!/^codex-cli 0\.153\.\d+$/.test(result.codexVersion))
            throw new Error(
              'Supported native contract is Codex 0.153.x; found ' + result.codexVersion,
            );
          break;

...
import { renderResult } from '../lib/presentation.js';
...
describe('native local refresh', () => {
  let root, home, codexHome, repoRoot, manifestFile, catalogFile, mapping, configFile;
  let env, options, calls, installed, hook, failure, version;
  const now = new Date('2026-09-16T18:30:00Z');
  const readManifest = async () => JSON.parse(await readFile(manifestFile, 'utf8'));
  async function native(argv) {
    calls.push(argv);
    if (hook) {
      const replacement = await hook(argv);
      if (replacement) return { argv, exitCode: 0, stderr: '', ...replacement };
    }
    if (failure?.(argv))
  ...
  const run = (extra = {}) => refreshPlugin({ ...options, ...extra }, { env, native, now });
  beforeEach(async () => {
  ...
  it('reinstalls stale and successive payloads without release bumps or same-second cache reuse', async () => {
    ...
    assert.equal(first.ok, true, first.issue);
    ...
    assert.equal(second.ok, true, second.issue);
  ...
  it('previews the manifest edit and pending validation without a process or writes', async () => {
    ...
    assert.equal(
      result.plan.find((step) => step.operation === 'edit-manifest').after,
  ...
  it('returns the actual native child exit code and effects through CLI JSON', async () => {
    ...
    const cli = fileURLToPath(new URL('../bin/codex-tools.js', import.meta.url));

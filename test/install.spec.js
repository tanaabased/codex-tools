...
const entry = (name = 'sample', source = './plugins/sample') => ({
  name,
...
describe('local installation (Me link-preservation and catalog reconciliation)', () => {
  ...
  const run = (extra = {}) => installPlugin({ ...options, ...extra }, { env, native });
  const catalog = (plugins = [], extra = {}) =>
  ...
  it('rejects unsafe or malformed source prerequisites before effects', async () => {
    for (const value of [
      ...
      await assert.rejects(run(), (error) => error.source?.valid === false);
    }

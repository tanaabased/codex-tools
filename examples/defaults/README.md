...
printf '%s' "$output" | EXPECTED_SOURCE="$PWD/source" EXPECTED_HOME="$root/home/.codex" bun -e 'const r = await Bun.stdin.json(); if (!r.ok || r.status !== "current" || r.source.path !== process.env.EXPECTED_SOURCE || r.codexHome !== process.env.EXPECTED_HOME || r.marketplace !== "personal") process.exit(1)'
```
